...
var VENDOR_DEVICE_LIST = [
  ...
  { vid: 0x8087, pid: 0x0aaa }, // Intel 9460/9560
...
BluetoothHciSocket.prototype.bindUser = function (devId) {
  ...
  if (
    ...
    if (
      process.env.BLUETOOTH_HCI_SOCKET_USB_BUS &&
    ...
    this._usbDevice = VENDOR_DEVICE_LIST.map((d) =>
      usb.findByIds(d.vid, d.pid)
    ).find((d) => d != null);
  }
...
BluetoothHciSocket.prototype.getDeviceList = function () {
  return usb
    ...
    .filter((dev) => {
      return (
        VENDOR_DEVICE_LIST.findIndex((d) => {
          return (
            dev.deviceDescriptor.idVendor == d.vid &&
            dev.deviceDescriptor.idProduct == d.pid
          );
        }) !== -1
      );
    })
...
BluetoothHciSocket.prototype.onHciEventEndpointData = function (data) {
  ...
  if (pktLen <= this._hciEventEndpointBuffer.length - 2) {
    ...
    if (
      this._mode === "raw" &&
      buf.length === 6 &&
      ("0e0401030c00" === buf.toString("hex") ||
        "0e0402030c00" === buf.toString("hex"))
    ) {
    ...
    // fire event
    this.emit("data", Buffer.concat([new Buffer([HCI_EVENT_PKT]), buf]));

    // reset buffer
    this._hciEventEndpointBuffer = this._hciEventEndpointBuffer.slice(
      pktLen + 2
    );
  }
};
...
BluetoothHciSocket.prototype.onAclDataInEndpointData = function (data) {
  ...
  if (pktLen <= this._aclDataInEndpointBuffer.length - 4) {
    ...
    // fire event
    this.emit("data", Buffer.concat([new Buffer([HCI_ACLDATA_PKT]), buf]));

    // reset buffer
    this._aclDataInEndpointBuffer = this._aclDataInEndpointBuffer.slice(
      pktLen + 4
    );
  }
};
